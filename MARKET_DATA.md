...
- GitHub Actions 在工作日北京时间交易时段约每 5 分钟尝试采集。定时任务可能延迟或失败，页面显示快照交易日、采集时间、过期状态；页面每分钟检查新快照。公开快照通过 Git 提交留存历史。
...
如需换数据供应商，只要脚本继续生成该 JSON 结构，前端不需要改动。注意示例仅展示格式，并非真实行情。首次定时任务成功后才会出现 `market-data` 分支与快照；页面在此之前明确显示暂无数据。
