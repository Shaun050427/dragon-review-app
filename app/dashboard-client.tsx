...
const defaultPayload = (): Payload => ({
  fields: {
    ...
    rivalSectors: "", regulatoryFacts: "", candidate: "", candidateRole: "未确定", allowedSetup: "",
...
const disciplineChecks = [
  ["disc_plan", "昨晚有书面预案"], ["disc_one", "只盯唯一候选"],
...
export function DashboardClient() {
  ...
  const gatePassed = gateChecks.every(([key]) => payload.checks[key]) && f.tradeMode !== "空仓观察" && Boolean(f.candidate) && Number(f.maxPosition) > 0;
    ...
    <section className="workspace">
      ...
      <Tabs defaultValue="plan" className="main-tabs"><TabsList className="tab-list"><TabsTrigger value="plan">今晚计划</TabsTrigger><TabsTrigger value="opening">9:30—10:00</TabsTrigger><TabsTrigger value="review">盘后复盘</TabsTrigger><TabsTrigger value="history">历史统计</TabsTrigger><TabsTrigger value="rules">龙空龙铁律</TabsTrigger></TabsList>
        <TabsContent value="plan" className="panel-grid">
          ...
          <Card className="span-8"><CardHeader><CardTitle>盘面事实</CardTitle></CardHeader><CardContent className="form-grid two"><Field label="指数 / 成交额 / 增减"><Textarea value={f.marketFacts} onChange={(e) => updateField("marketFacts", e.target.value)} /></Field><Field label="涨跌停 / 炸板 / 昨日溢价"><Textarea value={f.sentimentFacts} onChange={(e) => updateField("sentimentFacts", e.target.value)} /></Field><Field label="完整梯队"><Textarea value={f.ladder} onChange={(e) => updateField("ladder", e.target.value)} /></Field><Field label="竞争板块与资金流向"><Textarea value={f.rivalSectors} onChange={(e) => updateField("rivalSectors", e.target.value)} /></Field><Field label="监管 / 公告 / 催化事实"><Textarea value={f.regulatoryFacts} onChange={(e) => updateField("regulatoryFacts", e.target.value)} /></Field></CardContent></Card>
          <Card className="span-7"><CardHeader><CardTitle>唯一主候选</CardTitle></CardHeader><CardContent className="form-grid two"><Field label="股票与代码"><Input value={f.candidate} onChange={(e) => updateField("candidate", e.target.value)} /></Field><Field label="角色"><Select value={f.candidateRole} onValueChange={(v) => updateField("candidateRole", v)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{["未确定","发育期潜在龙","成型总龙","加速龙头","首次健康分歧","老总龙超跌反抽"].map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></Field><Field label="明日只允许的买点"><Input value={f.allowedSetup} onChange={(e) => updateField("allowedSetup", e.target.value)} /></Field><Field label="最大仓位 %"><Input type="number" min="0" max="40" value={f.maxPosition} onChange={(e) => updateField("maxPosition", e.target.value)} /></Field><Field label="成为龙头的三条证据"><Textarea value={f.leaderEvidence} onChange={(e) => updateField("leaderEvidence", e.target.value)} /></Field><Field label="三条证伪条件"><Textarea value={f.invalidations} onChange={(e) => updateField("invalidations", e.target.value)} /></Field></CardContent></Card>
        ...
        <TabsContent value="opening" className="panel-grid"><Card className="span-12"><CardHeader><CardTitle>30分钟时间锁</CardTitle></CardHeader><CardContent className="time-grid"><div><strong>9:30—9:35</strong><span>只观察</span><p>总龙承接、竞价兑现、梯队与竞争板块。非瞬间满足全部预案，不抢第一笔。</p></div><div><strong>9:35—9:50</strong><span>唯一执行窗</span><p>只执行昨晚唯一候选；被证伪，今天直接空仓。</p></div><div><strong>9:50—10:00</strong><span>锁单与退出</span><p>处理撤单或卖出并记录；10:00后不临时开新仓。</p></div></CardContent></Card>
          ...
          <Card className={`span-5 gate-card ${gatePassed ? "pass" : "stop"}`}><CardHeader><CardTitle>下单授权闸门</CardTitle></CardHeader><CardContent className="check-list">{gateChecks.map((item) => <CheckItem key={item[0]} item={item} checked={Boolean(payload.checks[item[0]])} onChange={(v) => updateCheck(item[0], v)} />)}<div className="gate-result">{gatePassed ? <><Check />允许执行：仅限 {f.candidate}，仓位≤{f.maxPosition}%</> : <><AlertTriangle />默认不下单：条件尚未全部满足</>}</div></CardContent></Card>
        </TabsContent>
