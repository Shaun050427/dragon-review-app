...
export function DashboardClient() {
  ...
  const save = useCallback(async (force = false) => {
    ...
    try {
      ...
      const document: ReviewDocument = { ...latest.document, schemaVersion: 6, updatedAt: now, records: { ...latest.document.records, [dateRef.current]: { payload: payloadRef.current, updatedAt: now, device: activeConfig.device } } };
  ...
  const exportCloud = () => {
    const blob = new Blob([JSON.stringify({ version: 6, source: "github", exportedAt: new Date().toISOString(), records }, null, 2)], { type: "application/json" });
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <Tabs defaultValue="plan" className="main-tabs"><TabsList className="tab-list"><TabsTrigger value="plan">今晚计划</TabsTrigger><TabsTrigger value="cycle">周期机会</TabsTrigger><TabsTrigger value="opening">9:30—10:00</TabsTrigger><TabsTrigger value="review">盘后复盘</TabsTrigger><TabsTrigger value="limit">涨停复盘图</TabsTrigger><TabsTrigger value="history">历史统计</TabsTrigger><TabsTrigger value="rules">龙空龙铁律</TabsTrigger></TabsList>
